...
export function AiReportScreen() {
  ...
  const removeHistoryItem = async (generatedAt: string) => {
    const next = history.filter((item) => item.generatedAt !== generatedAt);
  ...
  const labels = {
    ...
    aiUsed: isArabic ? "مولّد بالذكاء الاصطناعي" : "Generated by AI",
    aiFallback: isArabic ? "قالب ذكي احتياطي" : "Smart fallback template",
    historyTitle: isArabic ? "سجل التقارير" : "Report history",
    historyClear: isArabic ? "مسح السجل" : "Clear history",
    historyRemove: isArabic ? "حذف" : "Remove",
    historyEmpty: isArabic ? "لا توجد تقارير محفوظة" : "No saved reports",
  };
          ...
          <View style={styles.historyHeader}>
            ...
            >
              ...
              <View style={styles.historyRow}>
                ...
                <Pressable onPress={() => void removeHistoryItem(item.generatedAt)}>
                  <Text style={[styles.link, { color: colors.danger }]}>{labels.historyRemove}</Text>
                </Pressable>
              </View>
            </Pressable>
          ))}
